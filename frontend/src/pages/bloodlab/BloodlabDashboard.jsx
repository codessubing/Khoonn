      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {criticalStock > 0 && (
          <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 flex items-center gap-2">
            ...
            <div>
              <p className="font-semibold text-amber-700 dark:text-amber-300 text-xs">Low Stock Alert</p>
              <p className="text-amber-700/80 dark:text-amber-400/80 text-xs">
                {criticalStock} blood type{criticalStock > 1 ? "s" : ""} have low inventory
              </p>
            </div>
          </div>
        )}
        ...
        {lab && (
          <div className="bg-card border border-border rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              <LabInfo icon={<Mail className="w-3.5 h-3.5" />} label="Email" value={lab.email} />
              <LabInfo icon={<Phone className="w-3.5 h-3.5" />} label="Phone" value={lab.phone} />
              <LabInfo 
        ...
        <div className="grid grid-cols-1 gap-4">
          {/* Blood Stock Section */}
          <Section title="Blood Inventory" icon={<Droplet className="w-4 h-4 text-primary" />} subtitle="Current stock levels">
            {stock.length > 0 ? (
              ...
              <EmptyState icon={<Droplet className="w-6 h-6" />} message="No blood stock data" />
            )}
          </Section>

          {/* Recent Camps Section */}
          <Section title="Recent Camps" icon={<Calendar className="w-4 h-4 text-primary" />} subtitle="Latest organized camps">
            {dashboard?.recentCamps?.length > 0 ? (
              ...
              <EmptyState icon={<Calendar className="w-6 h-6" />} message="No recent camps" />
            )}
          </Section>
        </div>

        {/* Access History Section */}
        <Section title="Access History" icon={<Shield className="w-4 h-4 text-primary" />} subtitle="Recent login activity">
          {loginHistory.length > 0 ? (
            ...
            <EmptyState icon={<LogIn className="w-6 h-6" />} message="No login history" />
          )}
        </Section>

        {/* Activity History Section */}
        {lab?.history?.length > 0 && (
          <Section title="Recent Activity" icon={<Activity className="w-4 h-4 text-primary" />} subtitle="Laboratory activities">
            <div className="space-y-2">
    ...
    <div className="bg-card border border-border rounded-lg p-4 hover:shadow-sm transition-all duration-300">
      ...
      {trend && (
        <div className="flex items-center gap-1 mt-3 pt-3 border-t border-border">
          ...
          <span className="text-[10px] font-medium text-green-600 dark:text-green-400">+{trend}%</span>
        </div>
      )}
    </div>
  );
...
const BloodStockItem = ({ bloodType, quantity, critical = false }) => (
  <div className="flex items-center justify-between p-2.5 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
    <div className="flex items-center gap-2.5">
      <div className={`p-1.5 rounded-lg shrink-0 ${critical ? "bg-destructive/10 text-destructive" : "bg-green-500/10 text-green-600 dark:text-green-400"}`}>
        <Droplet className="w-3.5 h-3.5" />
      </div>
      <span className="font-medium text-foreground text-sm">{bloodType}</span>
    </div>
    <div className="text-right shrink-0">
      <span className={`font-bold text-sm ${critical ? "text-destructive" : "text-foreground"}`}>
        {quantity} units
      </span>
      {critical && <p className="text-[10px] text-destructive mt-0.5">Low stock</p>}
    </div>
  </div>
);
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
      <div className="flex-1 min-w-0 pr-3">
        <h4 className="font-medium text-foreground text-sm truncate">{camp.title}</h4>
...
const HistoryItem = ({ history, isLogin = false }) => {
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case "Stock Update": return <Droplet className="w-3.5 h-3.5" />;
      case "Blood Camp": return <Calendar className="w-3.5 h-3.5" />;
  ...
  return (
    <div className="flex items-center justify-between p-2.5 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
      <div className="flex items-center gap-2.5">
        <div className={`p-1.5 rounded-lg shrink-0 ${getColor(eventType)}`}>
          {getIcon(eventType)}
        </div>
        <div>
