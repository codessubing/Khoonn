...
export const getBloodLabDashboard = async (req, res) => {
  try {
    ...
    const [camps, stock, facility] = await Promise.all([
      BloodCamp.find({ hospital: labId }).sort({ createdAt: -1 }), // Note: 'hospital' field refers to the organizing facility (Lab)
    ...
    res.json({
      success: true,
      stats: { totalCamps, upcomingCamps, completedCamps, totalDonors, totalUnits },
...
export const getBloodLabHistory = async (req, res) => {
  try {
    ...
    const logins = (lab.history || [])
      ...
      .map((login) => ({ date: login.date, ip: login.description || "Unknown" }));

    res.json({ success: true, activity, logins });
  } catch (error) {
  ...
  try {
    ...
    if (missing.length) {
      return res.status(400).json({
        success: false,
        message: `Missing required fields: ${missing.map((f) => f.name).join(", ")}`,
      });
    }
    ...
    res.status(201).json({ success: true, message: "Blood camp created successfully", data: camp });
  } catch (error) {
...
export const updateBloodCamp = async (req, res) => {
  try {
    ...
    if (expectedDonors) camp.expectedDonors = Number(expectedDonors);
...
export const updateCampStatus = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: `Camp status updated to ${status}`, data: camp });
  } catch (error) {
...
export const addBloodStock = async (req, res) => {
  try {
    ...
    if (!bloodType || !quantity || Number(quantity) <= 0) {
      return res.status(400).json({ success: false, message: "Please provide valid bloodType and quantity" });
    }
    ...
    res.json({ success: true, message: "Blood stock added successfully", data: stock });
  } catch (error) {
...
export const removeBloodStock = async (req, res) => {
  try {
    ...
    if (!stock) {
      return res.status(404).json({ success: false, message: `No stock found for blood type ${bloodType}` });
    }

    if (stock.quantity < Number(quantity)) {
      return res.status(400).json({ success: false, message: `Insufficient stock. Available: ${stock.quantity} units` });
    }
    ...
    res.json({ success: true, message: "Blood stock removed successfully", data: { bloodType, remainingQuantity: stock.quantity } });
  } catch (error) {
...
export const updateBloodRequestStatus = async (req, res) => {
  try {
    ...
    if (action === "accept") {
      const labStock = await Blood.findOne({ bloodLab: labId, bloodGroup: request.bloodType });
    ...
    res.status(200).json({ success: true, message: `Request ${action}ed successfully`, data: request });
